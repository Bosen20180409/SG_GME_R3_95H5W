...
uint8_t Setup0_Process(void)
{
  ...
  {  
    pInformation->USBbmRequestType = USB->SETUP[0];  // bmRequestType
    pInformation->USBbRequest = USB->SETUP[1];       // bRequest */
    pInformation->USBwValue = (USB->SETUP[3])|(USB->SETUP[2]<<8);   // wValue
    pInformation->USBwIndex  = (USB->SETUP[5])|(USB->SETUP[4]<<8);  // wIndex
    pInformation->USBwLength = (USB->SETUP[6])|(USB->SETUP[7]<<8);  // wLength
  }
